...
export default async function HistoryPage({
  ...
  const mealKeys = Array.from(byMeal.keys());
    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="flex items-center justify-between gap-4">
          <Link
            href={`/history?date=${prevDate(date)}`}
          ...
          <Link
            href={isToday ? "/history" : `/history?date=${nextDate(date)}`}
      ...
      {entries.length === 0 ? (
        <Card>
          ...
          {!isToday && (
            <Link href="/" className="mt-2 block text-sm text-zinc-300 underline">
              Go to today to log meals →
            </Link>
          )}
        </Card>
      ) : (
        mealKeys.map((mealKey) => {
          const mealEntries = byMeal.get(mealKey) ?? [];
              ...
              <div className="space-y-2">
                {mealEntries.map((e) => {
                  ...
                  return (
                    <div
                      key={e.id}
                      className="flex flex-col gap-2 rounded-lg border border-zinc-800 bg-zinc-950/20 p-3 md:flex-row md:items-center md:justify-between"
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-xs text-zinc-400 tabular-nums">
                          {n ? (
                            <>
                              {round0(n.kcal)} kcal · {round1(n.protein_g)}P /{" "}
                              {round1(n.carbs_g)}C / {round1(n.fat_g)}F
                            </>
                          ) : "—"}
                        </div>
                        <form action={deleteLogEntry}>
                          <input type="hidden" name="id" value={e.id} />
                          <button className="rounded-lg border border-zinc-800 px-2 py-1 text-xs text-zinc-200 hover:bg-zinc-900">
                            Delete
                          </button>
                        </form>
                      </div>
